...
try:
    ...
    raise ImportError("database_connection.py not found. Please ensure it exists with the PostgresDB class.")
...
async def startup_event():
    ...
    This provides a default connected state.
    """
    print("Application startup: attempting to connect to the database...")
    if db.connect():
        print("Successfully connected to the database on startup.")
    else:
        print("Warning: Failed to connect to the database on startup. Use the /connect endpoint to try again.")
...
async def shutdown_event():
    ...
    """
    print("Application shutdown: disconnecting from the database...")
    db.disconnect()
...
async def connect_to_db() -> Dict[str, str]:
    ...
    if db.connect():
        ...
        raise HTTPException(status_code=500, detail="Failed to connect to the database. Check server logs for details.")

@app.post("/disconnect", tags=["Connection Management"], summary="Disconnect from the database")
async def disconnect_from_db() -> Dict[str, str]:
    """
    Closes the connection to the PostgreSQL database.
    """
    if not db.is_connected:
        return {"status": "success", "message": "Already disconnected from the database."}
    if db.disconnect(): # disconnect should ideally always return True or handle its errors.
        return {"status": "success", "message": "Successfully disconnected from the database."}
    else:
        # This path might be less common if db.disconnect() is robust
        raise HTTPException(status_code=500, detail="An error occurred during disconnection. Check server logs.")

@app.get("/status", tags=["Connection Management"], summary="Check database connection status")
...
@app.get("/tables", response_model=List[str], tags=["Database Operations"], summary="Get all table names")
...
@app.get("/tables/{table_name}/columns", response_model=List[str], tags=["Database Operations"], summary="Get all columns for a table")
async def get_all_columns(table_name: str, db_instance: PostgresDB = Depends(get_database_instance)):
    """
    ...
    if not columns and table_name not in db_instance.get_all_tables(): # Additional check if columns list is empty
